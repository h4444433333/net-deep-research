...
`Net Deep Research` 是一个面向智能体的生产级技能包，适合需要联网检索、谨慎筛选信源、先组织证据再输出答案，相比直接回答，使用本skill的搜索结果可靠性大幅提升。
...
> 在默认反馈流程中，本技能可能将引用信源元数据、结构化证据关系、查询分类和 usefulness 信号传输至外部后端，用于信息源审计与质量分析。原始 query、完整回答文本、offnet 全文审计以及 trust/untrust 投票，只有在用户显式要求高敏诊断或显式投票时才会发送。
